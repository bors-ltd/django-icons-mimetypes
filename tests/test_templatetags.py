...
class IconTestCase(TestCase):
    def test_icon_mimetype(self):
        self.assertEqual(
            mimetypes.icon("image/jpeg"), "/static/mimetypes/64x64/image-x-generic.png"
        )

    def test_icon_mimetype_size(self):
        self.assertEqual(
            mimetypes.icon("image/jpeg", size="32x32"),
...
class TagTestCase(TestCase):
    ...
    def test_mimetype(self):
        self.assertEqual(
            self.render("""{% icon "image/jpeg" %}"""),
            "/static/mimetypes/64x64/image-x-generic.png",
        )

    def test_mimetype_size(self):
        self.assertEqual(
            self.render("""{% icon "image/jpeg" "32x32" %}"""),
            "/static/mimetypes/32x32/image-x-generic.png",
        )

    def test_mimetype_size_var(self):
        self.assertEqual(
            self.render("""{% icon "image/jpeg" size %}""", size="16x16"),
            "/static/mimetypes/16x16/image-x-generic.png",
        )

    def test_mimetype_var(self):
        self.assertEqual(
            self.render("""{% icon mimetype %}""", mimetype="image/jpeg"),
            "/static/mimetypes/64x64/image-x-generic.png",
        )

    def test_mimetype_var_size(self):
        self.assertEqual(
            self.render("""{% icon mimetype "16x16" %}""", mimetype="image/jpeg"),
            "/static/mimetypes/16x16/image-x-generic.png",
        )
    ...
    def test_mimetype_var_size_var(self):
        self.assertEqual(
            self.render(
                """{% icon mimetype size %}""", mimetype="image/jpeg", size="24x24"
            ),
    ...
    def test_mimetype_as_varname(self):
        self.assertEqual(
            self.render("""{% icon "application/pdf" as pdf_icon %}"""), ""
        )
        self.assertEqual(
            self.context["pdf_icon"], "/static/mimetypes/64x64/application-pdf.png"
        )

    def test_mimetype_size_as_varname(self):
        self.assertEqual(
            self.render("""{% icon "application/pdf" "32x32" as pdf_icon %}"""), ""
        )
        self.assertEqual(
            self.context["pdf_icon"], "/static/mimetypes/32x32/application-pdf.png"
        )

    def test_mimetype_size_var_as_varname(self):
        self.assertEqual(
            self.render(
                """{% icon "application/pdf" size as pdf_icon %}""", size="16x16"
            ),
            "",
        )
        self.assertEqual(
            self.context["pdf_icon"], "/static/mimetypes/16x16/application-pdf.png"
        )

    def test_mimetype_var_as_varname(self):
        self.assertEqual(
            self.render(
                """{% icon mimetype as pdf_icon %}""", mimetype="application/pdf"
            ),
            "",
        )
        self.assertEqual(
    ...
    def test_errors(self):
        ...
        self.assertRaises(
            TemplateSyntaxError, self.render, """{% icon "image/jpeg" as %}"""
        )
        self.assertRaises(
            TemplateSyntaxError, self.render, """{% icon "image/jpeg" "32x32" as %}"""
        )
